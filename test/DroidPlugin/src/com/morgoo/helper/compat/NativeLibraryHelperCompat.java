...
public class NativeLibraryHelperCompat {
    ...
    private static int copyNativeBinariesBeforeL(File apkFile, File sharedLibraryDir) {
        try {
            ...
            return (Integer) MethodUtils.invokeStaticMethod(nativeLibraryHelperClass(), "copyNativeBinariesIfNeededLI", args);
        ...
        try {
            ...
            if (isVM64()) {
                if (Build.SUPPORTED_64_BIT_ABIS.length > 0) {
                    ...
                    int abiIndex = (Integer) MethodUtils.invokeStaticMethod(nativeLibraryHelperClass(), "findSupportedAbi", handleInstance, Build.SUPPORTED_64_BIT_ABIS);
                    if (abiIndex >= 0) {
                ...
                if (Build.SUPPORTED_32_BIT_ABIS.length > 0) {
                    ...
                    int abiIndex = (Integer) MethodUtils.invokeStaticMethod(nativeLibraryHelperClass(), "findSupportedAbi", handleInstance, Build.SUPPORTED_32_BIT_ABIS);
                    if (abiIndex >= 0) {
            ...
            args[1] = sharedLibraryDir;
            args[2] = abi;
            return (Integer) MethodUtils.invokeStaticMethod(nativeLibraryHelperClass(), "copyNativeBinaries", args);
        } catch (NoSuchMethodException e) {
            e.printStackTrace();
        } catch (IllegalAccessException e) {
            e.printStackTrace();
